...
app.post('/vehicles', (req, res) => {
  const { marque, modele, annee, plaque, client_id } = req.body;
  const sql = 'INSERT INTO vehicles (marque, modele, annee, plaque, client_id) VALUES (?, ?, ?, ?, ?)';
  db.query(sql, [marque, modele, annee, plaque, client_id], (err, result) => {
    if (err) {
      console.error(err);
      res.status(500).send('Server error');
      return;
...
app.put('/vehicles/:id', (req, res) => {
  const { id } = req.params;
  const { marque, modele, annee, plaque, client_id } = req.body;
  const sql = 'UPDATE vehicles SET marque = ?, modele = ?, annee = ?, plaque = ?, client_id = ? WHERE id = ?';
  db.query(sql, [marque, modele, annee, plaque, client_id, id], (err, result) => {
    if (err) {
      console.error(err);
      res.status(500).send('Server error');
      return;
